...
\begin{figure}[htp]
\centering
\includegraphics[height=7cm,width=0.6\textwidth]{figure4_semantic_model}
\caption{用户语义轨迹描述模型}
\label{fig:semantic_model}
\end{figure}
通过停留点检测算法（这里加上引用，因为是人家的）检测的用户轨迹中的停留点，我们通过构建轨迹的表示模型，针对每个用户利用轨迹表示模型来描述该用户的所有轨迹信息。为了方便的表示一个用户在纵向时间段内和横向的空间中的活动，我们采用一种三维的层次化的GPS 轨迹数据描述模型。如图4所示，在这个三维的数据结构中
\section{小结}
